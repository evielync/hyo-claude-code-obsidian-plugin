...
// mobile — plan content instead comes from the Write tool call that
// created it (handled inline where this is called), so this always
// reports null on mobile.
    ...
    (event: any) => {
      ...
      if (event.type === "control_request") {
        ...
        if (toolName === "ExitPlanMode") {
          const planContent = readPlanFile(undefined);
